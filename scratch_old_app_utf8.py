...
top_col1.title("≡ƒö¼ Hidden Scientific Links Discovery Engine")
...
tab_discovery, tab_explorer, tab_ingestion = st.tabs([
    "≡ƒÜÇ Agentic Discovery", 
    "≡ƒîÉ 2D Knowledge Graph Explorer", 
    "≡ƒôä Local Paper Ingestion"
])
            ...
            for event in agent_app.stream(initial_state, config, stream_mode="updates"):
                for node_name, state_update in event.items():
                    ...
                    if node_name == "grounding":
                        status.update(label="Γ£à Phase 1: Grounding Complete. Searching topology...")
                        concepts = state_update.get('grounded_concepts', [])
                        st.write(f"**Grounded Concepts:** `{', '.join(concepts)}`")
                        
                    # --- PHASE 2: DISCOVERY ---
                    elif node_name == "discovery":
                        status.update(label="Γ£à Phase 2: Structural Discovery Complete. Validating metrics...")
                        ...
                        for b in bridges:
                            if "type" in b and "name" in b:
                                st.caption(f"Γå│ {b['type']}: {b['name']}")
                            elif "shared_method" in b:
                                st.caption(f"Γå│ Method: {b['shared_method']}")
                            
                    # --- PHASE 3: VALIDATION ---
                    elif node_name == "validation":
                        status.update(label="Γ£à Phase 3: Validation Complete. Reviewing findings...")
                        links = state_update.get('validated_links', [])
                        st.write("Calculated Adamic-Adar scores and temporal trends.")
                        for l in links:
                            st.caption(f"Γå│ Bridge: {l.get('bridge', 'Unknown')}")
                            
                    # --- PHASE 3.5: SELF-CORRECTION ---
                    elif node_name == "self_correction":
                        status.update(label="Γ£à Phase 3.5: Peer Review Complete. Synthesizing hypothesis...")
                    ...
                    # --- PHASE 4: SYNTHESIS ---
                        ...
                        status.update(label="Γ£à Phase 4: Synthesis Complete!", state="complete")
                        final_text = state_update.get("final_hypothesis", "")
                        
                    # Catch the early exit event
                    elif node_name == "early_exit":
                        status.update(label="≡ƒ¢æ Discovery Halted: No connections found.", state="complete")
                        final_text = state_update.get("final_hypothesis", "")
        ...
        for bridge in all_candidate_bridges:
            if bridge.get("type") == "SEMANTIC_BRIDGE":
                ...
                if node_a and node_b and not kg.graph.has_edge(node_a, node_b):
                    ...
                    st.toast("≡ƒöÑ New Semantic Bridge injected into Knowledge Graph!", icon="≡ƒÜÇ")

        # Display the final hypothesis
        st.markdown("### ≡ƒÆí Final Scientific Hypothesis")
        ...
        for uploaded_pdf in uploaded_pdfs:
            with st.status(f"Processing {uploaded_pdf.name}...", expanded=True) as status:
                ...
                try:
                    ...
                    st.write(f"Γ£à Extracted {len(pages)} pages ({len(full_text.split())} words).")
                    ...
                    st.write(f"Γ£à Metadata identified: **{paper_title}** ({paper_year})")
